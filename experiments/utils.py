...
def create_ann_focal(input_dimension):
    """
    This function describes the archictecture and the compiling process of our artifical neural network with focal loss. 
...
def worker_supervised_experiments(i, splits, model_name, features, target):
  split = splits[i]
  ...
  if model_name == "logreg":
     ...
     model = LogisticRegression(random_state=42, max_iter=1000, class_weight="balanced")
  ...
  elif model_name == "gb_balanced":
      model = GradientBoostingClassifier(random_state=42)
      y_train_list = list(y_train)
      sample_weight_vector = compute_sample_weight(class_weight = "balanced", y = y_train_list)
        
  elif model_name == "ann":
      model = create_ann(X_train.shape[1])
    
  elif model_name == "ann_focal":
      model = create_ann_focal(X_train.shape[1])
    
  elif model_name == "ann_balanced_keras":
      ...
      class_weight_keras = {0: weight_for_0, 1: weight_for_1}
  ...
  if model_name == "ann" or model_name == "ann_focal":
      ...
      start_training = time.time()
      model.fit(X_train, y_train, epochs = 50, class_weight = class_weight_keras)
      end_training = time.time()
      ...
      start_training = time.time()
      model.fit(X_train, y_train, sample_weight=sample_weight_vector)
      end_training = time.time()
