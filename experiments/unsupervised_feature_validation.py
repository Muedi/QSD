...
preprocessed_data = preprocessed_data.loc[preprocessed_data["assay"] == assay].reset_index(drop= True) # filter assay type
...
custom_order = ["KNN", "LOF", "IForest", "AutoEncoder", "ECOD"]
df_summary_unsupervised_results = (df_auc_results.groupby("Model")[["AUC_ROC", "AUC_PR"]]).agg(["mean", "std"]).reindex(custom_order)
